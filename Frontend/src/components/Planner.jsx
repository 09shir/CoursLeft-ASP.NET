...
class Planner extends Component {
    ...
    deleteTerm = () => {
        ...
        // find all courses in that term
        var coursesID = []
        axios
            .get("/api/courses").catch(function (error){
                console.log("Failed to get all courses:" + error)})
            .then((res) => {
                res.data.map((c) => {
                    console.log(c)
                    if (c.idTerm == lastTerm.id){
                        coursesID.push(c.id)
                    }
                })
                console.log(coursesID)
                // delete all courses in that term before deleting term
                coursesID.map((ID) => {
                    axios
                        .delete(`/api/courses/${ID}`).catch(function (error){
                            console.log("Failed to delete a course:" + error)})
                        .then(
                            axios
    ...
    renderCourses = (term) => {
        const items = this.state.courses.filter((item) => item.idTerm === term.id)
